...
class SearchStudentResultController extends Controller
{
    public function index(Request $request){
        ...
        if ($student == null) {
            return redirect()->back()->with('error', 'No student found with the number '.$request->std_id);
        }
    ...
    private function fetchStudentResult($student){
        $student = StudentDetail::with('batch', 'year')->where('std_id', $student->std_id)->first();
        ...
        foreach($batch_years as $batch_year){
            ...
            foreach ($courses as $course) {
                ...
                if ($course->course != null) {
                    ...
                    $main_exams = MainExam::with(['main_exam_student_results' => function($query)use($student){ $query->where('std_id', $student->std_id); },
                                            'main_exam_student_results.main_exam'])
                                        ->with(['sub_exam_student_results' => function($query)use($student){ $query->where('std_id', $student->std_id); },
                                            'sub_exam_student_results.sub_exam_eval'])
                                        ->with(['sub_exam_reset_student_results' => function($query)use($student){ $query->where('std_id', $student->std_id); },
                    ...
                    $reset_exams = MainExamReset::with(['main_exam_reset_student_results' => function($query)use($student){ $query->where('std_id', $student->std_id); },
                                            'sub_exam_reset_student_results' => function($query)use($student) { $query->where('std_id', $student->std_id); },
                                            'sub_exam_reset_student_results.sub_reset_exam_eval'])
                                        ->where('course_id', $course->course->course_id)
                                        ->where('batch_id', $batch_year->batch->batch_id)
                                        ->where('year_id', $batch_year->year->semester_id)
        ...
        $current_year_courses = BatchCourse::with(['course' ])
            ...
            ->where('semester_id', $student->std_semester_id)
            ->get();
